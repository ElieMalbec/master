...
int colMaxMid(int arr[][4], int row, int col, int mid, int &index){
  int max = 0;
  for (int i(0); i < row; i++){
     if (max < arr[i][mid]){
       max = arr[i][mid];
...
int peak(int arr[][4], int row, int col){
  int midCol = col/2;
  //std::cout << midCol << std::endl;
  int rowMidCol = 0;
  int maxMidCol = colMaxMid(arr, row, col, midCol, rowMidCol);

  if (maxMidCol >= arr[rowMidCol][midCol-1] && max >= arr[rowMidCol][midCol+1]){
    return maxMidCol;
  }
  if (maxMidCol < arr[rowMidCol][midCol-1]){
    //return peak(arr, row, col, );
  }
  if (maxMidCol < arr[rowMidCol][midCol+1]){
...
int main(){
  ...
  std::cout << peak(arr, 4, 4) << std::endl;
}
